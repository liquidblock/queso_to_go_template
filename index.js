...
const global_lang = {
  ...
  command_add: settings.commands.add[0],
  command_back: settings.commands.back[0],
  command_remove: settings.commands.remove[0],
  command_level: settings.commands.level[0]
};
...
const noarg_command = (message, commands) => {
...
async function HandleMessage(message, sender, respond) {
  ...
  if (noarg_command(message, settings.commands.open) && sender.isBroadcaster) {
    queue_open = true;
    respond(__mf('queue.open', {sender: sender.displayName, ...global_lang}));
  } else if (noarg_command(message, settings.commands.close) && sender.isBroadcaster) {
    queue_open = false;
  ...
  } else if (noarg_command(message, settings.commands.level) && sender.isBroadcaster) {
  ...
  } else if (noarg_command(message, settings.commands.next) && sender.isBroadcaster) {
    level_timer.restart();
    level_timer.pause();
    let next_level = await quesoqueue.next();
    respond(next_level_message(next_level, sender.displayName));
  } else if (noarg_command(message, settings.commands.subnext) && sender.isBroadcaster) {
    level_timer.restart();
    level_timer.pause();
    let next_level = await quesoqueue.subnext();
    respond(next_level_message(next_level, sender.displayName));
  } else if (noarg_command(message, settings.commands.modnext) && sender.isBroadcaster) {
    level_timer.restart();
    level_timer.pause();
    let next_level = await quesoqueue.modnext();
    respond(next_level_message(next_level, sender.displayName));
  } else if (noarg_command(message, settings.commands.random) && sender.isBroadcaster) {
    level_timer.restart();
    level_timer.pause();
    let next_level = await quesoqueue.random();
    respond(next_level_message(next_level, sender.displayName));
  } else if (noarg_command(message, settings.commands.subrandom) && sender.isBroadcaster) {
    level_timer.restart();
    level_timer.pause();
    let next_level = await quesoqueue.subrandom();
    respond(next_level_message(next_level, sender.displayName));
  } else if (noarg_command(message, settings.commands.modrandom) && sender.isBroadcaster) {
    level_timer.restart();
    level_timer.pause();
    let next_level = await quesoqueue.modrandom();
    respond(next_level_message(next_level, sender.displayName));
  } else if (noarg_command(message, settings.commands.punt) && sender.isBroadcaster) {
    level_timer.restart();
    level_timer.pause();
  ...
  } else if (noarg_command(message, settings.commands.current)) {
  ...
  } else if (noarg_command(message, settings.commands.position)) {
    respond(await position_message(await quesoqueue.position(sender.displayName), sender.displayName));
  } else if (noarg_command(message, settings.commands.start) && sender.isBroadcaster) {
    level_timer.resume();
    respond(__mf('timer.start', {sender: sender.displayName, ...global_lang}));
  } else if (noarg_command(message, settings.commands.resume) && sender.isBroadcaster) {
    level_timer.resume();
    respond(__mf('timer.resume', {sender: sender.displayName, ...global_lang}));
  } else if (noarg_command(message, settings.commands.pause) && sender.isBroadcaster) {
    level_timer.pause();
    respond(__mf('timer.pause', {sender: sender.displayName, ...global_lang}));
  } else if (noarg_command(message, settings.commands.restart) && sender.isBroadcaster) {
    level_timer.restart();
    respond(__mf('timer.restart', {sender: sender.displayName, ...global_lang}));
  } else if (noarg_command(message, settings.commands.restore) && sender.isBroadcaster) {
    quesoqueue.load();
    respond(level_list_message(sender.displayName, quesoqueue.current(), await quesoqueue.list()));
  } else if (noarg_command(message, settings.commands.clear) && sender.isBroadcaster) {
    quesoqueue.clear();
    respond(__mf('queue.clear', {sender: sender.displayName, ...global_lang}));
  } else if (noarg_command(message, settings.commands.lurk)) {
    twitch.setToLurk(sender.username);
    respond(__mf('queue.lurk', {sender: sender.displayName, ...global_lang}));
  } else if (noarg_command(message, settings.commands.back)) {
    if (twitch.notLurkingAnymore(sender.username)) {
      respond(__mf('queue.back', {sender: sender.displayName, ...global_lang}));
    }
  } else if (noarg_command(message, settings.commands.order)) {
